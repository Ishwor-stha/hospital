...
module.exports.getPatientByPatientId = async (req, res, next) => {
    try {
        ...
        // no id on parameter
        if (!req.params.id) return next(new errorHandling("Empty id parameter: Ensure you're sending the correct information.", 400));
        // from url
        const patientId = req.params.id;
        // validate patient id
        if (!patientIdValidation(patientId)) return next(new errorHandling("The patient id entered is invalid. Kindly correct it.", 400));
        ...
        // no patient detail
        if (!patientDetail || patientDetail <= 0) return next(new errorHandling("No patient record found in the database.", 404));
...
module.exports.postPatient = async (req, res, next) => {
    try {
        ...
        if (!upload || Object.keys(upload).length <= 0) return next(new errorHandling("Creation of patient account was not successful. Please retry.", 500))
        // send response
        res.status(200).json({
            status: true,
...
module.exports.patientLogin = async (req, res, next) => {
    try {
        ...
        if (!patient || Object.keys(patient).length <= 0) return next(new errorHandling("No patient record found in the database from this email.", 404));
...
module.exports.updatePatient = async (req, res, next) => {
    try {
        ...
        if (Object.keys(req.body).length === 0) {
            return next(new errorHandling("Empty body: Ensure you're sending the correct information.", 400));
        }
        if (req.body.email) {
        ...
        // no patient detail
        if (!getDetail) return next(new errorHandling("No patient record found in the database.", 404));
        // 
        ...
        if (!updatedPatient ||Object.keys(updatedPatient).length<=0) return next(new errorHandling("Updating the patient record is not sucessfull,Please try again.", 500));
...
module.exports.deletePatient = async (req, res, next) => {
    try {
        ...
        if (!deletePatient) return next(new errorHandling("Error while deleting patient.Please try again later.", 500));
        // send sucess message
        res.status(200).json({
            stauts: true,
...
module.exports.forgetPassword = async (req, res, next) => {
    try {
        ...
        //email not provided
        if (!email) return next(new errorHandling("Empty email: Ensure you're sending the correct information.", 400));
        // validate email
        if (!emailValidation(email)) return next(new errorHandling("The email address entered is invalid. Kindly correct it.", 400));
        // check details on db
        const checkPatient = await patientModel.findOne({ email }, "name")
        // no details on db
        if (!checkPatient || Object.keys(checkPatient).length <= 0) return next(new errorHandling("No patient record found in the database.", 404));
        ...
        // update fails
        if (!update || Object.keys(update).length <= 0) return next(new errorHandling("Something went wrong while sending reset link .Please try again later", 500));
        // message part
...
module.exports.resetPassword = async (req, res, next) => {
    try {
        ...
        // update fail
        if (!update) return next(new errorHandling("Something went wrong while updating password.Please try agan.", 500));
